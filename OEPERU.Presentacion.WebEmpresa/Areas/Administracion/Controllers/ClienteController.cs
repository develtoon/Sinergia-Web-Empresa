...
namespace OEPERU.Presentacion.WebEmpresa.Controllers
{
    ...
    public class ClienteController : Controller
    {
        ...
        public ClienteController(OEPERUApiClient oeperuClient, IHttpContextAccessor accessor)
        {
        ...
        [ValidateAntiForgeryToken]
        [AuthorizationApiFilter]
        public async Task<JsonResult> GetList(string texto, int estado = 0, string ordenamiento = "", int pagina = 1, int tamanio = 0)
        {
        ...
        {
            string url = "";
            url = string.Format("{0}?estado={1}&texto={2}&pagina={3}&ordenamiento={4}&tamanio={5}", OEPERUApiName.EmpresaClientes,
                estado, texto, pagina, ordenamiento, tamanio);
        ...
        {
            string url = string.Format("{0}/{1}", OEPERUApiName.EmpresaClientes, id);
            Dictionary<string, object> response = await _oeperuClient.GetAsync(url, HttpContext, urlApiAdministracion);
        ...
        [ValidateAntiForgeryToken]
        [AuthorizationApiFilter]
        public async Task<JsonResult> Save([FromBody] ClienteInput input)
        {
            ...
            {
                string url = "";
                url = OEPERUApiName.EmpresaClientes;
        ...
        public async Task<JsonResult> Delete(string id)
        {
            ...
            url = string.Format(OEPERUApiName.ApiDelete,
                OEPERUApiName.EmpresaClientes, id);
        ...
        {
            string url = "";
            url = string.Format("{0}/exportar?estado={1}&texto={2}&pagina={3}&ordenamiento={4}&tamanio={5}", OEPERUApiName.EmpresaClientes,
                estado, texto, pagina, ordenamiento, tamanio);
            ...
            using (MemoryStream memoryStream = new MemoryStream())
            {
                ...
                var filename = $"Empresa_Cliente_{new DateTime().ToString("yyyyMMdd_HHmmss")}.xlsx";
